...
# توکن Railway قبلاً فقط روی دیسک ذخیره می‌شد. روی هر پلتفرمی که دیسک بین
# دیپلوی‌ها پایدار نیست (Vercel، Render بدون disk، Railway بدون Volume) توکن
# گم می‌شد و پروکسی‌های MTProto بعد از هر دیپلوی از بین می‌رفتند. حالا:
#   env RAILWAY_API_TOKEN → کش حافظه (که از Redis پر می‌شود) → دیسک
_token_cache: Optional[str] = None


def _redis_token_sync() -> Optional[str]:
    return _token_cache


async def load_token_async() -> Optional[str]:
    """نسخه‌ی async که Redis را هم چک می‌کند و کش حافظه را پر می‌کند.
    main.py این را موقع راه‌اندازی صدا می‌زند تا نسخه‌ی sync هم درست جواب دهد."""
    global _token_cache
    env_tok = os.environ.get("RAILWAY_API_TOKEN", "").strip()
    if env_tok:
        _token_cache = env_tok
        return env_tok
    val = await rt.kv_get(rt.K_RAILWAY_TOKEN)
    if val:
        _token_cache = val.strip()
        return _token_cache
    return load_token()


async def save_token_async(token: str):
    global _token_cache
    token = token.strip()
    _token_cache = token
    if not await rt.kv_set(rt.K_RAILWAY_TOKEN, token):
        save_token(token)
    elif rt.DISK_PERSISTENT:
        save_token(token)


async def clear_token_async():
    global _token_cache
    _token_cache = None
    await rt.kv_del(rt.K_RAILWAY_TOKEN)
    clear_token()
...
def has_saved_token() -> bool:
    if os.environ.get("RAILWAY_API_TOKEN", "").strip() or _token_cache:
        return True
    try:
...
def save_token(token: str):
    ...
    if not rt.DISK_PERSISTENT:
        # دیسک موقتی است؛ نوشتن روی آن فقط توهم پایداری می‌سازد.
        return
    try:
