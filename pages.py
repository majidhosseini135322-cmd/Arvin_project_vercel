...
async function fetchStats(){
  try{
    ...
    if(redisEl){
      if(d.redis_connected){redisEl.style.color='var(--green-t)';redisEl.textContent=d.storage_backend==='upstash'?'● متصل (Upstash)':'● متصل';}
      else if(d.redis_configured){redisEl.style.color='var(--red-t)';redisEl.textContent=d.serverless?'● وصل نیست (دیتا ذخیره نمی‌شود!)':'● وصل نیست (فایل محلی جایگزین)';}
      else{redisEl.style.color='var(--t3)';redisEl.textContent='تنظیم نشده';}
    }
    if(storageEl){const bm={upstash:'Upstash Redis',redis:'Redis',file:'JSON File (/data)',none:'⚠️ ناپایدار (بدون ذخیره‌سازی)'};storageEl.textContent=bm[d.storage_backend]||(d.redis_connected?'Redis':'JSON File (/data)');}
